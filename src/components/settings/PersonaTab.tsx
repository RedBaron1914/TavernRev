  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-lg font-bold text-gray-300">{t('yourPersonas', 'Your Personas')}</h3>
        <button
        ...
        >
          <Plus size={16} /> {t('addPersona', 'Add Persona')}
        </button>
      </div>
      <div className="grid gap-4 w-full overflow-hidden">
          ...
          >
            ...
            <div className="flex-1 min-w-0 overflow-hidden">
              ...
              <p className="text-sm text-gray-500 truncate">
                {p.description || t('noDescriptionProvided', 'No description provided.')}
              </p>
            </div>
            <button
              onClick={(e) => {
              ...
              title={p.is_default ? "Default Persona" : t('setAsDefault', 'Set as Default')}
            >
              <Star
                size={18}
                className={
        ...
        {(!userPersonas || userPersonas.length === 0) && (
          <div className="text-center py-12 text-gray-600 border-2 border-dashed border-gray-800 rounded-2xl">
            ...
            <p>{t('noPersonasCreatedYet', 'No personas created yet.')}</p>
          </div>
        )}
      </div>
    </div>
