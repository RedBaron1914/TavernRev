...
export const DebugConsole = ({
  ...
  const tabs: {id: LogCategory, label: string, icon: any}[] = [
    { id: 'all', label: t('all', 'All'), icon: Terminal },
    { id: 'ai', label: t('aiPrompt', 'AI Prompt'), icon: Brain },
    { id: 'error', label: t('errors', 'Errors'), icon: ShieldAlert },
    { id: 'database', label: 'DB', icon: Database },
    { id: 'system', label: t('system', 'System'), icon: Terminal },
  ];
    ...
    <div className="fixed inset-0 bg-black/95 z-[9999] flex flex-col font-mono text-xs text-white p-4 animate-in fade-in duration-200">
      <div className="flex justify-between items-center border-b border-gray-700 pb-2 mb-4 bg-black/50 p-2 rounded shrink-0">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => logger.clear()} className="p-2 hover:bg-white/10 rounded text-red-400" title={t('clearAll', 'Clear All')}><Trash2 size={18}/></button>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-1 select-text p-2 custom-scrollbar border border-white/5 rounded-xl bg-black/30">
        {filteredLogs.length === 0 && <div className="text-gray-600 italic text-center mt-10">{t('noLogsFoundForThisFilter', 'No logs found for this filter...')}</div>}
        {filteredLogs.map((log, i) => (
          <div key={i} className={`break-words whitespace-pre-wrap border-b border-white/5 pb-1 mb-1 ` + (
            ...
            <span className="text-gray-500 opacity-50 select-none text-[9px]">{t('timestamp', '[{{timestamp}}]', { timestamp: log.timestamp })}</span>{' '}
