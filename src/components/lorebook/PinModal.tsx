    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-gray-900 border border-white/10 rounded-2xl p-5 w-72 space-y-3 shadow-2xl">
        <div className="flex justify-between items-center">
          <h3 className="text-xs font-bold text-white">{t('linkLorebook', 'Link Lorebook')}</h3>
          <button onClick={onClose} className="p-2 text-gray-500 hover:text-white rounded-lg transition">
            <X size={14} />
          </button>
        </div>
        <p className="text-[11px] text-gray-400">
          {t('link', 'Link')} <span className="text-white font-semibold">{modal.bookName}</span> {t('to', 'to:')}
        </p>
        <div className="flex gap-2">
          <button onClick={() => onPin("card")}
            ...
            <User size={13} /> {t('character', 'Character')}
          </button>
          <button onClick={() => onPin("chat")}
            ...
            disabled={!chatId}>
            <MessageSquare size={13} /> {t('chat', 'Chat')}
          </button>
        </div>
        <label className="flex items-center gap-2.5 p-2.5 bg-amber-500/5 border border-amber-500/20 rounded-xl cursor-pointer">
          ...
          />
          <div>
            <div className="text-[10px] font-bold text-amber-300">{t('excludeFromGlobalPool', 'Exclude from global pool')}</div>
            <div className="text-[9px] text-amber-400/60">{t('wontAppearInTheGlobalTabAfterLinking', 'Won\'t appear in the Global tab after linking')}</div>
          </div>
        </label>
      </div>
    </div>
