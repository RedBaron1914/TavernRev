...
pub fn get_lorebooks(conn: &Connection) -> Result<Vec<Lorebook>> {
    let mut stmt = conn.prepare("SELECT id, name, description, is_global FROM lorebooks")?;
    let rows = stmt.query_map([], |row| {
        Ok(Lorebook {
            id: row.get(0)?,
            name: row.get(1)?,
...
pub fn get_lore_entries(conn: &Connection, book_id: i64) -> Result<Vec<LoreEntry>> {
    let mut stmt = conn.prepare("SELECT id, book_id, keys, content, enabled, constant, priority, probability, position, depth FROM lore_entries WHERE book_id = ?1")?;
...
pub fn get_active_lore_entries(
    ...
    let mut stmt = conn.prepare(
        "SELECT e.id, e.book_id, e.keys, e.content, e.enabled, e.constant, e.priority, e.probability, e.position, e.depth, 'character' as source
         ...
         WHERE cl.character_id = ?1 AND e.enabled = 1
         UNION
         ...
         WHERE cl.chat_id = ?2 AND e.enabled = 1
         UNION
...
pub fn get_chat_lorebooks(conn: &Connection, chat_id: i64) -> Result<Vec<i64>> {
